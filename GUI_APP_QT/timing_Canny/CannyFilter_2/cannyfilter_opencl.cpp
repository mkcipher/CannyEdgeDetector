...
uint8_t* CannyFilter_OpenCL::Detector(string Platform, uint8_t* input_image,unsigned long width_image,unsigned long height_image,
                                      unsigned char Min_Threshold, unsigned char Max_Threshold, long int& elapsedtime,
                                      unsigned int ReturnSelection){
    ...
    switch (ReturnSelection) {
        ...
        case 1:
            queue.enqueueReadBuffer(gauss_out, true, 0, size, h_outputGpu.data(),NULL,&READBUFFERTIME);
        break;

        case 2:
            queue.enqueueReadBuffer(sobel_out, true, 0, size, h_outputGpu.data(),NULL,&READBUFFERTIME);
        break;

        case 3:
            queue.enqueueReadBuffer(theta, true, 0, size, h_outputGpu.data(),NULL,&READBUFFERTIME);
        break;

        case 4:
            queue.enqueueReadBuffer(non_max_out, true, 0, size, h_outputGpu.data(),NULL,&READBUFFERTIME);
        break;
